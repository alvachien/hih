			...
			case HIH_ACTIVITY_GETLIST:
				{
					...
					if (! IsNullOrEmptyString ($lrnawdrst[0])) {
						...
						if (count($rsttable) > 0) {
							$footerar = array();
							$footerar[] = array('displayas' => 'Avg', 'score' => round($lrnawdrst[2][0]['avg'], 2));
							$footerar[] = array('displayas' => 'Total', 'score' => round($lrnawdrst[2][0]['total'], 2));
							$prejson['footer'] = $footerar;
						} else {
							$footerar = array();
							$footerar[] = array('displayas' => 'Avg', 'score' => 0);
							$footerar[] = array('displayas' => 'Total', 'score' => 0);
							$prejson['footer'] = $footerar;								
						}
